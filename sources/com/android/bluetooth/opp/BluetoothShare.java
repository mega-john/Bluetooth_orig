...
public final class BluetoothShare implements BaseColumns {
    ...
    public static boolean isStatusSuccess(int status) {
        return status >= STATUS_SUCCESS && status < 300;
    }

    public static boolean isStatusError(int status) {
        return status >= STATUS_BAD_REQUEST && status < 600;
    }

    public static boolean isStatusClientError(int status) {
        return status >= STATUS_BAD_REQUEST && status < 500;
    }
    ...
    public static boolean isStatusCompleted(int status) {
        return (status >= STATUS_SUCCESS && status < 300) || (status >= STATUS_BAD_REQUEST && status < 600);
    }
}
